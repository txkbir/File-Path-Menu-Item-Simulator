...
void listAll(const MenuItem* m, string path) { // two-parameter overload
    const vector<MenuItem*>* ptrToVec = m->menuItems();
    if (ptrToVec == nullptr) return;
    for (int i = 0; i < ptrToVec->size(); ++i) {
        MenuItem* ptrToMenu = (*ptrToVec)[i];
        cout << path + ptrToMenu->name() << endl;
        listAll(ptrToMenu, path + ptrToMenu->name() + "/");
    }
}
